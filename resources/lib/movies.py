...
class Movies(object):
    ...
    def listing(self):
        ...
        if tag:
            ...
            label_prefix = ""
        all_items = [
            (label_prefix + self.addon.getLocalizedString(32028), "inprogress&mediatype=movies&tag=%s"
             % tag, Movies.ICON_IMAGE_DEFAULT),
            (label_prefix + self.addon.getLocalizedString(32038), "recent&mediatype=movies&tag=%s"
             % tag, Movies.ICON_IMAGE_DEFAULT),
            ...
            (label_prefix + self.addon.getLocalizedString(32048), "random&mediatype=movies&tag=%s"
             % tag, Movies.ICON_IMAGE_DEFAULT),
            (label_prefix + self.addon.getLocalizedString(32066), "unwatched&mediatype=movies&tag=%s"
             % tag, Movies.ICON_IMAGE_DEFAULT),
            (label_prefix + self.addon.getLocalizedString(32046), "top250&mediatype=movies&tag=%s"
             % tag, Movies.ICON_IMAGE_DEFAULT),
        ...
        if not tag:
            all_items += [
                (self.addon.getLocalizedString(32006), "similar&mediatype=movies&tag=%s"
                 % tag, Movies.ICON_IMAGE_DEFAULT),
                (self.addon.getLocalizedString(32076), "forgenre&mediatype=movies", Movies.ICON_IMAGE_DEFAULT),
                (xbmc.getLocalizedString(10134), "favourites&mediatype=movies&tag=%s" % tag, Movies.ICON_IMAGE_DEFAULT),
    ...
    def tagslisting(self):
        ...
        for item in self.metadatautils.kodidb.files("videodb://movies/tags"):
            details = (item["label"], "listing&mediatype=movies&tag=%s" % item["label"], Movies.ICON_IMAGE_TAGS)
    ...
    def playlistslisting(self):
        ...
        for item in self.metadatautils.kodidb.files("special://videoplaylists/"):
            ...
            details = (item["label"], "playlist&mediatype=movies&tag=%s" % tag_label, Movies.ICON_IMAGE_DEFAULT)
            all_items.append(create_main_entry(details))
        return all_items
